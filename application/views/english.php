	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function inturn(){
				...
				var englishsongnext=document.getElementById("englishsong"+nextid);
				englishsongnext.play();
			}
			...
			function cycleplay(){
				...
				if(nextid==num){
					var englishsongfirst=document.getElementById("englishsong0");
					englishsongfirst.play();
				}
			}
			//随机播放函数
			function randomplay(){
				 var i=parseInt(Math.random()*num);
				 var englishsongrandom=document.getElementById("englishsong"+i);
				 englishsongrandom.play();
			}
			
			for(var i=0;i<num;i++){

				$("#english_look_lyric_id"+i).mouseover(function(){
					$("#"+this.id).attr("class","look_lyric_over");
				});
				$("#english_look_lyric_id"+i).mouseleave(function(){
					$("#"+this.id).attr("class","look_lyric");
				});
				
				$("#english_look_lyric_id"+i).click(function(){
					var temp=this.id.substr(21);
					$('#english_myModal'+temp).modal('show');
				});
				var englishsong=document.getElementById("englishsong"+i);
				if(i<num){
					englishsong.addEventListener('ended',inturn);
				}
			}
			...
			$("#one_cycle_id").click(function(){
				...
				for(var i=0;i<num;i++){
					var englishsong=document.getElementById("englishsong"+i);
					englishsong.removeEventListener('ended', inturn);
					englishsong.removeEventListener('ended', cycleplay);
					englishsong.removeEventListener('ended', randomplay);
					englishsong.addEventListener('ended',one_cycle);
				//	alert("aaa");
				}
			});
			//顺序播放
			$("#inturn_id").click(function(){
					...
					for(var i=0;i<num;i++){
						var englishsong=document.getElementById("englishsong"+i);
						englishsong.removeEventListener('ended', one_cycle);
						englishsong.removeEventListener('ended', cycleplay);
						englishsong.removeEventListener('ended', randomplay);
						if(i<num){
							englishsong.addEventListener('ended',inturn);
			...
			$("#cycleplay_id").click(function(){
					...
					for(var i=0;i<num;i++){
						...
						englishsong.removeEventListener('ended', inturn);
						englishsong.removeEventListener('ended', randomplay);
						englishsong.addEventListener('ended',cycleplay);
					}
			});
			...
			$("#random_id").click(function(){
					...
					for(var i=0;i<num;i++){
						...
						englishsong.removeEventListener('ended', inturn);
						englishsong.removeEventListener('ended', cycleplay);
						englishsong.addEventListener('ended',randomplay);
					}
			});
		});
...
<div class="mymusic">
		<div class="mytitle">
		<br/>
		<div class="music_title">英文金曲</div>
		...
		<div class="music_content">
			<?php foreach ($english_result as $key=>$one_result){ ?>
			<div class="one_song">
				<div class="one_song_first">
				<h3><i class="icon-music"></i> <?php echo $one_result->name ?></h3>
				<div>
				<audio controls="true" preload="none" id="englishsong<?php echo $key?>">
				...
				<div class="look_lyric" id="english_look_lyric_id<?php echo $key?>"> 查看歌词</div><br/><br/>
			...
			<div class="modal fade" id="english_myModal<?php echo $key?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
