...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			//顺序播放函数
			function inturn(){
				var nextid=parseInt(this.id.substr(8))+1;
				var lovesongnext=document.getElementById("lovesong"+nextid);
				lovesongnext.play();
			}
			...
			function cycleplay(){
				...
				if(nextid==num){
					var lovesongfirst=document.getElementById("lovesong0");
					lovesongfirst.play();
				}
			}
			//随机播放函数
			function randomplay(){
				 ...
				 var lovesongrandom=document.getElementById("lovesong"+i);
				 lovesongrandom.play();
			}
			
			for(var i=0;i<num;i++){

				$("#love_look_lyric_id"+i).mouseover(function(){
					$("#"+this.id).attr("class","look_lyric_over");
				});
				$("#love_look_lyric_id"+i).mouseleave(function(){
					$("#"+this.id).attr("class","look_lyric");
				});
				
				$("#love_look_lyric_id"+i).click(function(){
					var temp=this.id.substr(18);
					$('#love_myModal'+temp).modal('show');
				});

				var lovesong=document.getElementById("lovesong"+i);
				if(i<num){
					lovesong.addEventListener('ended',inturn);
				}
			}
			...
			$("#one_cycle_id").click(function(){
				...
				for(var i=0;i<num;i++){
					var lovesong=document.getElementById("lovesong"+i);
					lovesong.removeEventListener('ended', inturn);
					lovesong.removeEventListener('ended', cycleplay);
					lovesong.removeEventListener('ended', randomplay);
					lovesong.addEventListener('ended',one_cycle);
				//	alert("aaa");
				}
			});
			//顺序播放
			$("#inturn_id").click(function(){
					...
					for(var i=0;i<num;i++){
						var lovesong=document.getElementById("lovesong"+i);
						lovesong.removeEventListener('ended', one_cycle);
						lovesong.removeEventListener('ended', cycleplay);
						lovesong.removeEventListener('ended', randomplay);
						if(i<num){
							lovesong.addEventListener('ended',inturn);
			...
			$("#cycleplay_id").click(function(){
					...
					for(var i=0;i<num;i++){
						...
						lovesong.removeEventListener('ended', inturn);
						lovesong.removeEventListener('ended', randomplay);
						lovesong.addEventListener('ended',cycleplay);
					}
			});
			...
			$("#random_id").click(function(){
					...
					for(var i=0;i<num;i++){
						...
						lovesong.removeEventListener('ended', inturn);
						lovesong.removeEventListener('ended', cycleplay);
						lovesong.addEventListener('ended',randomplay);
					}
			});
...
<div class="main">
	...
	<div class="mymusic">
		<div class="mytitle">
		<br/>
		<div class="music_title">情歌速递</div>
		...
		<div class="music_content">
			<?php foreach ($love_result as $key=>$one_result){ ?>
			<div class="one_song">
				<div class="one_song_first">
				<h3><i class="icon-music"></i> <?php echo $one_result->name ?></h3>
				<div>
				<audio controls="true" preload="none" id="lovesong<?php echo $key?>">
				...
				<div class="look_lyric" id="love_look_lyric_id<?php echo $key?>"> 查看歌词</div><br/><br/>
			...
			<div class="modal fade" id="love_myModal<?php echo $key?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
