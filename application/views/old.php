...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			//顺序播放函数
			function inturn(){
				var nextid=parseInt(this.id.substr(7))+1;
				var oldsongnext=document.getElementById("oldsong"+nextid);
				oldsongnext.play();
			}
			...
			function cycleplay(){
				...
				if(nextid==num){
					var oldsongfirst=document.getElementById("oldsong0");
					oldsongfirst.play();
				}
			}
			//随机播放函数
			function randomplay(){
				 ...
				 var oldsongrandom=document.getElementById("oldsong"+i);
				 oldsongrandom.play();
			}
			
			for(var i=0;i<num;i++){

				$("#old_look_lyric_id"+i).mouseover(function(){
					$("#"+this.id).attr("class","look_lyric_over");
				});
				$("#old_look_lyric_id"+i).mouseleave(function(){
					$("#"+this.id).attr("class","look_lyric");
				});
				
				$("#old_look_lyric_id"+i).click(function(){
					var temp=this.id.substr(17);
					$('#old_myModal'+temp).modal('show');
				});

				var oldsong=document.getElementById("oldsong"+i);
				if(i<num){
					oldsong.addEventListener('ended',inturn);
				}
			}
			...
			$("#one_cycle_id").click(function(){
				...
				for(var i=0;i<num;i++){
					var oldsong=document.getElementById("oldsong"+i);
					oldsong.removeEventListener('ended', inturn);
					oldsong.removeEventListener('ended', cycleplay);
					oldsong.removeEventListener('ended', randomplay);
					oldsong.addEventListener('ended',one_cycle);
				//	alert("aaa");
				}
			});
			//顺序播放
			$("#inturn_id").click(function(){
					...
					for(var i=0;i<num;i++){
						var oldsong=document.getElementById("oldsong"+i);
						oldsong.removeEventListener('ended', one_cycle);
						oldsong.removeEventListener('ended', cycleplay);
						oldsong.removeEventListener('ended', randomplay);
						if(i<num){
							oldsong.addEventListener('ended',inturn);
			...
			$("#cycleplay_id").click(function(){
					...
					for(var i=0;i<num;i++){
						...
						oldsong.removeEventListener('ended', inturn);
						oldsong.removeEventListener('ended', randomplay);
						oldsong.addEventListener('ended',cycleplay);
					}
			});
			...
			$("#random_id").click(function(){
					...
					for(var i=0;i<num;i++){
						...
						oldsong.removeEventListener('ended', inturn);
						oldsong.removeEventListener('ended', cycleplay);
						oldsong.addEventListener('ended',randomplay);
					}
			});
...
<div class="main">
	...
	<br/> 
		...
		<div class="mytitle">
		<br/>
		<div class="music_title">怀旧经典</div>
		...
		<div class="music_content">
			<?php foreach ($old_result as $key=>$one_result){ ?>
			<div class="one_song">
				<div class="one_song_first">
				<h3><i class="icon-music"></i> <?php echo $one_result->name ?></h3>
				<div>
				<audio controls="true" preload="none"  id="oldsong<?php echo $key?>">
				...
				<div class="look_lyric" id="old_look_lyric_id<?php echo $key?>"> 查看歌词</div><br/><br/>
			...
			<div class="modal fade" id="old_myModal<?php echo $key?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
